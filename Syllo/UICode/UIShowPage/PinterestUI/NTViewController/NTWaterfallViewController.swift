...
//    class var sharedInstance: NSInteger = 0 Are u kidding me?
    ...
    override func viewDidLoad() {
        ...
        self.view.backgroundColor = showBackgroundColor
        var index = 0
        while(index<14){
        ...
        collection.frame.origin = CGPoint(x: waterFallEdage, y: 0)
        collection.frame.size = CGSize(width: screenWidth - waterFallEdage*2, height: screenHeight)
        collection.backgroundColor = showBackgroundColor
    ...
    func collectionView(collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAtIndexPath indexPath: NSIndexPath) -> CGSize{
        ...
        return CGSizeMake(gridWidth, imageHeight + 50)
    }
    ...
    func viewWillAppearWithPageIndex(pageIndex : NSInteger) {
        ...
        let imageHeight = image.size.height*gridWidth/image.size.width * 2
        if imageHeight > 800 {//whatever you like, it's the max value for height of image
           position = .Top
        }
